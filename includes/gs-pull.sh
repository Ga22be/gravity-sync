...
function pull_gs_dnsmasq {
    ...
    then
        ...
        then
            ...
            MESSAGE="${UI_PULL_PRIMARY} ${UI_NAME[$1]}"
            echo_stat
            RSYNC_REPATH="rsync"
            RSYNC_SOURCE="${REMOTE_USER}@${REMOTE_HOST}:${RIHOLE_DIR}/dnsmasq.d-${DNSMAQ_FILES[$1]}.backup"
            RSYNC_TARGET="${LOCAL_FOLDR}/${BACKUP_FOLD}/${DNSMAQ_FILES[$1]}.pull"
            create_rsynccmd

            MESSAGE="${UI_REPLACE_SECONDARY} ${UI_NAME[$1]}"
            echo_stat
            sudo cp ${LOCAL_FOLDR}/${BACKUP_FOLD}/${DNSMAQ_FILES[$1]}.pull ${DNSMAQ_DIR}/${DNSMAQ_FILES[$1]} >/dev/null 2>&1
            error_validate

            validate_dnsmasq_permissions $1
        fi
    fi
}
...
function pull_gs {
    ...
    pull_gs_grav
    pull_gs_cust
    pull_gs_dnsmasq "CNAME"
    pull_gs_dnsmasq "GSLAN"
    pull_gs_reload
    md5_recheck
    backup_cleanup
